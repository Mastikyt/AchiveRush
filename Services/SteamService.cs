...
public class SteamService
{
    ...
    public async Task<List<SteamPlayerAchievement>> GetPlayerAchievements(string steamId, int appId)
    {
        ...
        var success = successElement.ValueKind switch
        {
            ...
            JsonValueKind.String => bool.TryParse(successElement.GetString(), out var parsedBool) && parsedBool,
            _ => false
        };
        ...
        foreach (var ach in achievementsElement.EnumerateArray())
        {
            ...
            {
                achieved = achievedEl.ValueKind switch
                {
                    JsonValueKind.True => true,
                    JsonValueKind.False => false,
                    JsonValueKind.Number => achievedEl.GetInt32() == 1,
                    JsonValueKind.String => achievedEl.GetString() == "1" ||
                                            bool.TryParse(achievedEl.GetString(), out var parsedBool) && parsedBool,
                    _ => false
                };
            }
        ...
        foreach (var ach in achievements.EnumerateArray())
        {
            ...
            if (ach.TryGetProperty("percent", out var percentElement))
            {
                if (percentElement.ValueKind == JsonValueKind.Number)
                    ...
                    double.TryParse(percentElement.GetString(), out percent);
            }
    ...
    {
        var key = _config["Steam:ApiKey"];
        var url = $"https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key={key}&steamid={steamId}&include_appinfo=1&include_played_free_games=1";
